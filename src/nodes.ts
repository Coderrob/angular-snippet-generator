...
export const findAssignedProperty = (
  ...
  /**
   * Checks if a property assignment matches the given name.
   * @param prop - The property to test.
   * @returns True if the property name matches.
   */
  const matchesName = (
  ...
  return node.properties.find(matchesName);
};
 ...
 */
export const getClassName = (node: ts.Node | undefined): string =>
...
export const getReferenceTypeName = (
  ...
  return resolveTypeReference(typeNode, sourceCode);
};
 ...
 */
export const isComponent = (node: ts.Decorator | undefined): boolean =>
  !!node &&
 ...
 */
export const isDirective = (node: ts.Decorator | undefined): boolean =>
  !!node &&
  ts.isCallExpression(node.expression) &&
 ...
 */
export const isPipe = (node: ts.Decorator | undefined): boolean =>
  !!node &&
  ts.isCallExpression(node.expression) &&
...
/**
 * Resolves the type name from a TypeReferenceNode.
